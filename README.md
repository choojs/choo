...
- __event based:__ an event system makes writing apps easy
- __small API:__ with only 6 methods there's not much to learn
- __minimal tooling:__ built for the `browserify` compiler
- __isomorphic:__ renders seamlessly in Node and browsers
...
Programming should be fun and light, not stern and stressful. It's
cool to be cute; using serious words without explaining them doesn't make for better results. If anything, it scares people off. Choo doesn't want to be scary. It wants to be nice, fun, and _casually_ the best choice around—_Real casually_.

Choo is architected with the philosophy that frameworks should be disposable, and components recyclable. Choo is against a web where walled gardens jealously compete with one another. By making the DOM the lowest common denominator, switching from one framework to another
becomes frictionless. Choo is modest in its design. It accepts it won't
be top of the class forever. It architected to be easy to toss out as it is to
pick up.

Choo doesn't believe that bigger is better. Big APIs, large complexities, long files see like omens of impending userland complexity. Choo is made so everyone on a team, no matter the size, can fully understand how an application is laid out. And once an application is built, Choo is designed to remain small, performant and easy to reason about. These decisions make it easy to debug code, provide better results
and put smiles on faces.

## Events

Choo is an event emitter, which is used for both application
logic and also to interface with the framework itself. [Nanobus](https://github.com/choojs/nanobus) is used to provide this functionality.
...
### The purpose of the emitter is two-fold

-  It allows wiring up application code together and splitting it off nicely.
-  It allows communicating with the __Choo__ framework itself. All events can be read as constants from `state.events`. 

### Choo ships with the following events built in

#### `'DOMContentLoaded'`|`state.events.DOMCONTENTLOADED`

Emitted when the DOM is ready. Similar to the DOM's
`'DOMContentLoaded'` event, except it will be emitted even if the listener is added _after_ the DOM became ready. Uses
[document-ready](https://github.com/bendrucker/document-ready) under the hood.

#### `'render'`|`state.events.RENDER`

Emitted to re-render the DOM. A common pattern is to
update the `state` object, and then emit the `'render'` event straight after.
Note that `'render'` will only have an affect once the `DOMContentLoaded` event
has been fired.

#### `'navigate'`|`state.events.NAVIGATE`

Emitted whenever routes change. This is triggered by `'pushState'`, `'replaceState'` or `'popState'`.

#### `'pushState'`|`state.events.PUSHSTATE`

Emitted to navigate to routes. New routes are added
to the browser's history stack, and will emit `'navigate'` and `'render'`.
Similar to [history.pushState](http://devdocs.io/dom/history_api).

#### `'replaceState'`|`state.events.REPLACESTATE`

Emitted to navigate to a new route. The new route replaces
the current entry in the browser's history stack, and will emit `'navigate'` and `'render'`. Similar to [history.replaceState](http://devdocs.io/dom/history#history-replacestate).

#### `'popState'`|`state.events.POPSTATE`

Emitted to navigate to a previous route. The new route
...
Emitted when the `document.title` needs to be updated.
It sets both `document.title` and `state.title`.  This value can be used
when server rendering to accurately include a `<title>` tag in the header.
It is derived from the [DOMTitleChanged event](https://developer.mozilla.org/en-US/docs/Web/Events/DOMTitleChanged).

## State

Choo comes with a shared state object. This object can be mutated. It is passed into the view functions whenever `'render'` is emitted. The state object comes with a few properties set.

When initializing the application, `window.initialState` is used to provision the initial state. This is useful when combined with server
rendering. See [server rendering](#server-rendering) for details.
...
them on `state.events`, it reduces the chance of typos, autocompletes
...
Choo is an application level framework. It takes care of
everything related to routing and pathnames.
...
Sometimes a route doesn't match, and displaying a page to handle it is desired.
This can be done by declaring `app.route('*', handler)` to handle all routes
...
When a hash is found, Choo checks if there's an available anchor on the same
page, and will scroll the screen to that position. Using both hashes in URLs and anchor links on the page is not recommended.
...
To navigate routes, `'pushState'`, `'popState'` or
`'replaceState'` can be emitted. See [#events](#events) for details.
...
server rendering, and providing an initial state on the client to create the exact same document is also known as "rehydration".
...
Choo is reasonably fast out of the box. Sometimes there might be a scenario
where a particular part of the UI slows down the application, and speeding it up is desired. 

### Here are some possible optimizations:

#### Caching DOM elements

Sometimes telling the algorithm to not evaluate certain nodes (and its
children) is desired. This can be because we're sure they haven't changed, or perhaps
...
It's common to work with lists of elements on the DOM. Adding, removing or reordering elements in a list can be expensive. To optimize this, an `id` attribute can be added to a DOM node. When reordering nodes, it will compare nodes with the same ID against each other, resulting in far fewer re-renders. This is especially potent when coupled with DOM node caching.
...
Choo uses the `require('assert')` module from Node core to provide helpful error
messages in development. In production, this should probably be scripted out using
[unassertify][unassertify].

To convert inlined HTML to valid DOM nodes Choo `require('bel')` is used. This has
overhead during runtime. For production environments, unwrap this
using [yo-yoify][yo-yoify].

Setting up browserify transforms can sometimes be a bit of hassle. To make this
more convenient, use [bankai][bankai] with `--optimize` to
...
Because the developer thought it sounded cute. Many other programs talk about being
_"performant"_, _"rigid"_, _"robust"_—while Choo aims to make programming light, fun and
non-scary.
:steam_locomotive::train::train::train:

### Is it called choo, choo.js or...?

It's called "Choo", though "choo-choo" or
"chugga-chugga-choo-choo" is fine, too. The only time "choo.js" is tolerated is if /
...
virtual nodes. It turns out that [browsers are good at
dealing with DOM nodes][morphdom-bench]. Choo offers the benefit of
working with _any_ library that produces valid DOM nodes. So to put a long
answer short: nanomorph is better than virtual-dom.

### How can Choo support older browsers?

Template strings aren't supported in all browsers, and parsing them creates
significant overhead. To optimize, it is recommended to run `browserify` with
[yo-yoify][yo-yoify] as a global transform or use [bankai][bankai] directly.
```sh
...
This section provides documentation on each function in __Choo__. It's
intended to be for technical reference. To learn Choo for the first time, consider reading through the [handbook][handbook]:sparkles:
...
Choo is independently funded. We fight for our users. This does mean however that we also have to spend time working contracts to pay the bills. This is where you can help: by chipping in you can ensure more time is spent improving Choo rather than dealing with distractions.

### Sponsors

Become a sponsor and help ensure the development of independent quality software. You can help us keep the lights on, bellies full and work days sharp and focused on improving the state of the web. [Become a
